...
activation_name = sys.argv[1] if len(sys.argv) > 1 else 'roswish'  # 'relu', 'gelu', 'silu', 'selu', 'mish', 'prelu', 'erfact', 'swish', 'psgu', or 'roswish'
...
class ErfAct(nn.Module):
    def __init__(self, n_hidden):
        ...
        self.a = nn.Parameter(torch.full((1, n_hidden, 1, 1), 0.75))
        self.b = nn.Parameter(torch.full((1, n_hidden, 1, 1), 0.75))
...
class Swish(nn.Module):
    def __init__(self, n_hidden):
        ...
        self.beta = nn.Parameter(torch.full((1, n_hidden, 1, 1), 1.00))
...
class PSGU(nn.Module):
    def __init__(self, n_hidden):
        ...
        self.beta = nn.Parameter(torch.full((1, n_hidden, 1, 1), 3.00))
...
class CNN(nn.Module):
    ...
    def select(self, activation_fn, n_hiddens):
        if activation_fn == ErfAct or Swish or PSGU or RoSwish:
